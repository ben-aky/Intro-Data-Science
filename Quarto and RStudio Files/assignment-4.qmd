...
-   Open coding-assignment-4.qmd. Make sure the document includes a title, your name, and the date. (1 point)
...
There are 236 observations in the study. There were 118 villages in the control and treatment group.

## Part 2: Calculate the Treatment Effect of a Variable (16 points)

Choose one of the variables. Calculate the means and treatment effect of your chosen variable. 

```{r}
means<-myData |>
  group_by(t) |>
  summarize(wmn_mean = mean(role_wmn, na.rm = TRUE))
means
```
```{r}
mean_ctr = means$wmn_mean[1]
mean_trt = means$wmn_mean[2]
teffect=mean_ctr-mean_trt
teffect
```
...
H0: mean_trt−mean_ctr=0
H1: mean_trt−mean_ctr≠0
...
```{r}
null_dist <- myData |>
  specify(response = role_wmn, explanatory = t) |>
...
```{r}
get_p_value(null_dist, obs_stat = teffect, direction = "both")
```
...
```{r}
visualize(null_dist) +
  shade_p_value(obs_stat = teffect, direction = "both") +
  labs(
...
Given the results of my analysis, we can reject the null hypothesis as the p-value is not statistically significant. This means the difference between the mean of the control and treatment groups effect on gender equality is statistically different from 0. The analysis thus proves that Community Driven Development (CDD) programs in Sierra Leone were correlated with improvments in gender equality in participatory decision making.

## Extra Credit

There are addtional outcome variables in the study that you did not select: get the p-value and visualize the treatment effect relative to the null distribution for these variables. 
